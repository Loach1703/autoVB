...
#### 2.3.3 aoa=(n1, n2, ...)
活性轨道原子序列。指定活性空间所在涉及的原子的索引。比如 `aoa=(1,2, 3,4)` 表示 1和2、3和4 分别构成对。原子编号从 1 开始，范围在 `[1, 分子总原子数]` 的整数之间。

#### 2.3.4 stru=n
配置将要在计算中生成的价键结构类型，可选值包括 `"full"`, `"cov"`, `"ion(a,b,c)"`, `"ion(a-b)"`。默认值为 `"full"`，具体含义参见 [XMVB 文档](https://xmvb.xmu.edu.cn/docs/xmvb/content.html#str-options)。
